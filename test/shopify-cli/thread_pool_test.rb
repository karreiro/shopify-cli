...
module ShopifyCLI
  class ThreadPoolTest < MiniTest::Test
    ...
    def test_schedule_with_a_recurring_job
      mutex = Mutex.new
      actual_number_of_executions = 0
      expected_number_of_executions = 3
      thread_pool = ShopifyCLI::ThreadPool.new

      job = RecurringJob.new do
        mutex.synchronize { actual_number_of_executions += 1 }
      end
      ...
      with_retries(Minitest::Assertion) do
        assert_equal(expected_number_of_executions, actual_number_of_executions)
      end

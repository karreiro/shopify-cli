      ...
      class JsonDeleteHandlerTest < Minitest::Test
        ...
        def test_enqueue_json_deletes_when_it_should_not_overwrite_json_files_and_strategy_is_delete
          ...
          @to_delete.each do |file|
            Forms::SelectDeleteStrategy.expects(:ask).with(@ctx, [], file: file).returns(stub(strategy: :delete))
          end

          Forms::ApplyToAll.expects(:new).with(@ctx, @to_delete.size).returns(stub(apply?: false, value: nil))
        ...
        def test_enqueue_json_deletes_when_it_should_not_overwrite_json_files_and_strategy_is_restore
          @overwrite_json = false
          @to_delete.each do |file|
            Forms::SelectDeleteStrategy.expects(:ask).with(@ctx, [], file: file).returns(stub(strategy: :restore))
          end
        ...
        def test_enqueue_json_deletes_when_it_should_not_overwrite_json_files_and_apply_to_all_is_enabled
          ...
          Forms::ApplyToAll.expects(:new).with(@ctx, @to_delete.size).returns(stub(apply?: true, value: :delete))

  ...
  module Theme
    class Syncer
      ...
      def initialize(ctx, theme:, include_filter: nil, ignore_filter: nil, overwrite_json: true, pull_interval: 0)
        ...
        @overwrite_json = pull_interval.zero?
      ...
      def report_error(operation, error_suffix = "")
        @error_checksums << @checksums[operation.relative_path]
      ...
      def enqueue(method, file)
        ...
        if ignore_operation?(operation)
          @ctx.debug("ignore #{operation.relative_path}")
          return
        end
      ...
      def union_merge(file)
        ...
        if file.text?
          ...
          new_content = Merger.union_merge(file, remote_content)

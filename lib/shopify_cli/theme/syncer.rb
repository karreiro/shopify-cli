...
module ShopifyCLI
  module Theme
    class Syncer
      ...
      def fetch_checksums!
        _status, _body, response = theme_api.get(
          path: theme_assets_path
        )
      ...
      def upload_theme!(delay_low_priority_files: false, delete: true, overwrite_json_files: true, &block)
        opts = { overwrite_json_files: overwrite_json_files }
        ...
        enqueue_json_updates(opts[:overwrite_json_files])
      ...
      def theme_api
        @theme_api ||= ThemeAdminAPI.new(@ctx, @theme)
      end
      ...
      def remote_text_content(file)
        _status, body, _response = theme_api.get(
          path: theme_assets_path,
      ...
      def update(file, opts = {})
        remote_merge = opts[:remote_merge]
        asset = asset_update_param(file, remote_merge)

        _status, body, response = theme_api.put(
          path: theme_assets_path,
        ...
        file.write(asset[:value]) if remote_merge
      ...
      def get(file, opts = {})
        _status, body, response = theme_api.get(
          path: theme_assets_path,
          query: URI.encode_www_form("asset[key]" => file.relative_path),
        )

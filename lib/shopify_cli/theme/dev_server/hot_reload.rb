    ...
    module DevServer
      class HotReload
        ...
        def notify_streams_of_file_change(modified, added, removed)
          ...
          remote_delete(files) unless deleted_files.empty?
        ...
        def remote_delete(files)
          files.each do |file|
            puts("delete file each -> file.relative_path #{file.relative_path}")
        ...
        def remote_reload(files)
          files.each do |file|
            puts("reload file each -> file.relative_path #{file.relative_path}")
